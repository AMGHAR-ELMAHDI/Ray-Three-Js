...
export default function ThreeScene() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      if (isDraggingRef.current) {
        ...
        if (productRef.current) {
          ...
          productRef.current.rotation.x = Math.max(
            -Math.PI / 3,
            Math.min(Math.PI / 3, productRef.current.rotation.x)
          );
        }
    ...
    const animate = () => {
      ...
      if (productRef.current) {
        if (!isDraggingRef.current) {
          ...
          if (
            ...
            productRef.current.rotation.y +=
              (targetRotationY - productRef.current.rotation.y) * 0.02;
            productRef.current.rotation.x +=
              (targetRotationX - productRef.current.rotation.x) * 0.02;

            // Auto rotation when idle
            if (
              Math.abs(targetRotationY) < 0.01 &&
              Math.abs(targetRotationX) < 0.01
            ) {
      ...
      if (sceneRef.current) {
        const pointLight = sceneRef.current.getObjectByName(
          "pointLight"
        ) as THREE.PointLight;
        if (pointLight) {
  ...
  return (
    <div
      ref={mountRef}
      className={`absolute inset-0 transition-opacity duration-2000 cursor-grab active:cursor-grabbing ${
        isLoaded ? "opacity-100" : "opacity-0"
      }`}
      style={{
        background:
          "linear-gradient(135deg, rgba(0,0,0,0.05) 0%, rgba(255,215,0,0.1) 50%, rgba(0,0,0,0.05) 100%)",
      }}
    />
  );
}
